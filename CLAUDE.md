...
**The graph does NOT produce the report.** `agent/graph.py` compiles only two nodes — `supervisor` and `dispatcher` — looping `START → supervisor → dispatcher → supervisor → … → END`. Synthesis is deliberately *not* a graph node. Both entry points (`run.py`, `api/main.py`) call `graph.invoke()`/`astream()` to gather tool results, then call `synthesis_node(result)` / `stream_synthesis(result)` separately. If you invoke the graph and expect `final_report` in the output, it won't be there — you must run synthesis yourself.
...
**Ordering constraint: `sec_edgar` must run before `rag_search`, never in the same iteration.** `rag_search` ingests the filing URLs that `sec_edgar` discovers, but tools in one dispatcher batch all see the same pre-batch state snapshot. The supervisor's system prompt forbids queuing them together, and `supervisor_node` has fallback logic that force-queues `rag_search` in the iteration after `sec_edgar` if Claude forgot. `run_rag_pipeline` also skips re-ingest when the store already holds chunks for the company.
